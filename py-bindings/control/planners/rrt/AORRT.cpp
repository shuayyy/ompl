...
void ompl::binding::control::initPlannersRrt_AORRT(nb::module_ &m)
{
    nb::class_<oc::AORRT, ob::Planner>(m, "AORRT")
        ...
        .def(
            "getPlannerData", [](const oc::AORRT &planner, ob::PlannerData &data)
            { planner.getPlannerData(data); }, nb::arg("data"))
        .def("setup", &oc::AORRT::setup)
        .def("getPreviousSolutionCost", &oc::AORRT::getPreviousSolutionCost);
}
