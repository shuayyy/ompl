...
ompl::control::AORRT::AORRT(const SpaceInformationPtr &si) : base::Planner(si, "AORRT")
{
    ...
    Planner::declareParam<bool>("intermediate_states", this, &AORRT::setIntermediateStates, &AORRT::getIntermediateStates,
                                "0,1");
}
...
void ompl::control::AORRT::pruneNN(base::Cost costBound)
{
    ...
    OMPL_INFORM("%s: Pruned %zu nodes above cost bound %.5f; %u remain.",
                getName().c_str(), removed, costBound.value(), (unsigned)nn_->size());
}
